...
func TestCalculateSlot(t *testing.T) {
	...
	testAddress2 := common.HexToAddress("0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
	testAddress3 := common.HexToAddress("0xcccccccccccccccccccccccccccccccccccccccc")
	validatorsWithStake[testAddress1] = uint256.NewInt(80)
	validatorsWithStake[testAddress2] = uint256.NewInt(10)
	validatorsWithStake[testAddress3] = uint256.NewInt(10)
...
func TestGetLeaderAtSlot(t *testing.T) {
	...
	assert.Equal(t, ls.GetLeaderAtSlot(uint256.NewInt(0)), testAddress1)
	assert.Equal(t, ls.GetLeaderAtSlot(uint256.NewInt(1)), testAddress2)
	assert.Equal(t, ls.GetLeaderAtSlot(uint256.NewInt(2)), testAddress3)
}
